module move_swap::move_swap {
  ...
  public struct MOVE_SWAP has drop {}

  public struct Pair has key {
    id: UID,
    balance_x: Balance<FAUCET_COIN>,
    balance_y: Balance<MY_COIN>,
    lp_cap: TreasuryCap<MOVE_SWAP>,
    lp_locked: Balance<MOVE_SWAP>
  }

  fun create_lp(otw: MOVE_SWAP, ctx: &mut TxContext): TreasuryCap<MOVE_SWAP> {
    let (treasury_cap, metadata) = coin::create_currency(
        otw, 
        6,
        b"LPC",
        b"LeDat LP Coin",
        b"The LP Coin for LeDat",
        option::some(url::new_unsafe_from_bytes(b"https://skillicons.dev/icons?i=react")), 
        ctx
    );
  ...
  fun init(otw: MOVE_SWAP, ctx: &mut TxContext){
  ...
  public entry fun add_liquidity(pair: &mut Pair, coin_x: &mut Coin<FAUCET_COIN>, max_amount_x: u64, coin_y: &mut Coin<MY_COIN>, max_amount_y: u64, ctx: &mut TxContext) {
    ...
    let liquidity = if (total_supply == 0) {
      let locked_liquidity = coin::mint<MOVE_SWAP>(
          &mut pair.lp_cap, 
          MINIMUM_LIQUIDITY, 
          ctx
      );
    ...
    coin::mint_and_transfer<MOVE_SWAP>(&mut pair.lp_cap, liquidity, tx_context::sender(ctx), ctx);
  }

  public entry fun remove_liquidity(pair: &mut Pair, coin_lp: Coin<MOVE_SWAP>, ctx: &mut TxContext) {
    let total_supply = coin::total_supply(&pair.lp_cap);
  ...
  public entry fun swap_to_y(pair: &mut Pair, coin_x: Coin<FAUCET_COIN>, min_amount_out: u64, ctx: &mut TxContext) {
    assert!(coin_x.value() > 0 && min_amount_out > 0, EZeroAmount);

    let reserve_x = pair.balance_x.value();
    let reserve_y = pair.balance_y.value();
    let amount_y_received = calculate_desired_amount_withdraw(reserve_x, reserve_y, coin_x.value());
  ...
  public entry fun swap_to_x(pair: &mut Pair, coin_y: Coin<MY_COIN>, min_amount_out: u64, ctx: &mut TxContext) {
    assert!(coin_y.value() > 0 && min_amount_out > 0, EZeroAmount);

    let reserve_x = pair.balance_x.value();
    let reserve_y = pair.balance_y.value();
    let amount_x_received = calculate_desired_amount_withdraw(reserve_y, reserve_x, coin_y.value());
